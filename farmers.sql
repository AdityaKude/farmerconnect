...
CREATE TABLE 'expertadvice' (
    id INT(11) AUTO_INCREMENT PRIMARY KEY,  -- Unique ID for each expert advice (INT(11) can store up to 2 billion values)
    user_id INT(11) NOT NULL,  -- Foreign key to the user requesting advice (INT(11) can store user IDs)
    query TEXT NOT NULL,  -- Query submitted by the user (TEXT can store up to 65,535 characters)
    status VARCHAR(50) NOT NULL DEFAULT 'pending',  -- Status of the request (max length 50 characters)
    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,  -- Timestamp of when the record was created
    FOREIGN KEY (user_id) REFERENCES users(id)  -- Assuming a 'users' table with column 'id'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

CREATE TABLE 'chatbot' (
    id INT(11) AUTO_INCREMENT PRIMARY KEY,  -- Unique ID for each chatbot conversation
    user_id INT(11) NOT NULL,  -- Foreign key to the user who is interacting with the chatbot
    message TEXT NOT NULL,  -- Message exchanged in the conversation (TEXT can store up to 65,535 characters)
    response TEXT NOT NULL,  -- Response from the chatbot (TEXT can store up to 65,535 characters)
    status VARCHAR(50) NOT NULL DEFAULT 'active',  -- Status of the conversation (max length 50 characters)
    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,  -- Timestamp of when the record was created
    FOREIGN KEY (user_id) REFERENCES users(id)  -- Assuming a 'users' table with column 'id'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
